...
def predictionSplittedTraining():
    ...
    y_o = toSample['ind'].fillna(0).values
    for train_index, test_index in skf.split(X_o, y_o):
        ...
        X = Xtest.drop(['ind'], 1).values
        X = imp.transform(X)
        res = finalForTest[['patient_id', 'ind', 'POD', 'maxCDN', 'CD']]
        res['pred'] = clf.predict(X)
    ...
    print('sentivity : '+str(sum(allIn['roundPred'][allIn['ind']==1]))+' / '+str(allIn[allIn['ind']==1].shape[0]))
    print('FP : '+ str(allIn[(allIn['roundPred']==1) & (allIn['ind']==0)].shape[0]))
    print('FN : '+ str(allIn[(allIn['roundPred']==0) & (allIn['ind']==1)].shape[0]))

    print('mat')
    print(str(allIn[(allIn['roundPred']==0) & (allIn['ind']==0)].shape[0]) + '      '+ str(allIn[(allIn['roundPred']==1) & (allIn['ind']==0)].shape[0]))
    print(str(allIn[(allIn['roundPred']==0) & (allIn['ind']==1)].shape[0]) + '      '+ str(allIn[(allIn['roundPred']==1) & (allIn['ind']==1)].shape[0]))
...
def predictionSplittedPrediction(patientId, traningResult):
    ...
    if(minRaxa[minRaxa['maxCDN']>=3].shape[0]>0):
        ...
        minRaxa = 1000
    forPat['MYCDN']= (forPat['pred']>0.4) *1
    ...
    for patient_id, pod, ind, cd, maxCdn ,pred, roundPred in list(forPat[['patient_id', 'POD', 'ind', 'CD', 'maxCDN','pred', 'roundPred']][forPat['patient_id'] ==patientId].values):
        podJSON = {}
        podJSON['POD'] = pod
        podJSON['ind'] = ind
        podJSON['CD'] = cd
