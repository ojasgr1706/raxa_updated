...
from CtakesResponseParsing import create_listFromCtakesResponse
import json
...
def concatPatientInList(sql_query, cursor, initial_list):
    cursor.execute(sql_query)
    res = pd.DataFrame(cursor.fetchall())
    res.columns = ['patient_id']
    final_list = initial_list.tolist() + np.unique(res['patient_id'].values).tolist()
    return final_list
...
def add_concept_form_db(pathToCCIs):
    ...
    if ('patient_id' not in listPatient.columns):
        ...
        patient_series = np.unique(listPatient['patient_id'].values)

    # Other patients from the database having diabetes
    sql_query = 'select distinct person_id from obs where concept_id IN (119481, 175);'
    final_patientlist = concatPatientInList(sql_query, cursor, patient_series.tolist())

    print(len(final_patientlist))

    # Other non-diabetic patient form the database
    sql_query = 'select distinct person_id from sri_non_diabetic_data'
    final_patientlist = concatPatientInList(sql_query, cursor, final_patientlist)
...
def update_extracted_value(single_todo_df, old_now, obs_ids):
    if obs_ids:
        cnx = mysql.connector.connect(user=userdb, password=pwddb, host=hostdb, database=databasedb)
        cursor = cnx.cursor()
...
def check_dataType_id(concept_id):
    sql_query = ('select distinct datatype_id from concept where concept_id = ' + str(concept_id))
    res = query(sql_query)

    res = list(res)
    data_type_id = res[0]
    data_type_id = list(data_type_id)[0]
    # print(data_type_id)
    return data_type_id

def remove_duplicate_mlob(res):
    tmp = []
    res_list = []
...
def check_concepts_from_mlob(res, old_now, single_todo_df):
    ...
    # res = remove_duplicate_mlob(res)

    for i in res:
        result_list = list(i)
        value_text = result_list[0]
        concept_id = result_list[2]
        datatype_id = result_list[3]
        concept_value = None
                ...
                if concept_value is not None:
                    ...
                    if key in listNumDic:
                        ...
                        if key.lower() == 'rt':
                            ...
                            if key.lower() == 'temp':
                                if concept_value < 40.0:
                                    print('Temp in Celcius Key: ' + key + ' Index ' + str(column_index) + ' Value: ' + str(
                                        concept_value) + " patientId " + patient_id)
                                    concept_value = (concept_value * 1.8) + 32
                            print('Key: ' + key + ' Index ' + str(column_index) + ' Value: ' + str(concept_value) + " patientId " + patient_id )
...
def extract_value(string):
    ...
    if isinstance(string, str):
        if string is not None or str(string) is not 'nan' :
...
# Not used because it is not adding values to the DataFrame
def fetch_mlob_for_obs_ids(obsIds):
    if not obsIds:
        ...
        cnx = mysql.connector.connect(user=userdb, password=pwddb, host=hostdb, database=databasedb)
        cursor = cnx.cursor()
        sql_query = ('SELECT value_text from mlob where value_text is not null and value_text != "" and obs_id in (' + ','.join([str(f) for f in obsIds]) + ')')
        # print(sql_query)
        cursor.execute(sql_query)
        res = cursor.fetchall()
...
def fetch_concepts_from_freetext(obs_ids):
    if not obs_ids:
        return []
    else:

        # Get the unique obs_ids
        obs_ids = unique(obs_ids)
        res = []

        for i in obs_ids:
            res += create_listFromCtakesResponse(i)
        ...
        if not df.empty:
            ...
            def extract_dictionary(row, res_ctakes_list_arg, key_value_dic_arg):
                ...
                if key not in list_to_avoid:
                    ...
                    key_value = extract_value(value[1])
