...
def getDataFromPatientId(isAnalyseCtakes):
    ...
    if isAnalyseCtakes:
        ...
        res['detected_character_count'], res['ctakes_text'] = zip(*res['value_text'].apply(lambda x: get_count_structure_Text(x)))
...
def detectFromCtakesJson():
    df = pd.read_pickle('ctakes_extraction.pckl')
    ...
    def get_count_structure_Text(row):
        ...
        # Get the ctakes resList of Containing obj of Class CtakesData
        ctakes_res_list = parse_res_to_list(ctakes_json)



    df[['value_text', 'ctakes_text']][0:5].apply(lambda x: get_count_structure_Text(x), axis=1)


if __name__ == '__main__':
    res = getDataFromPatientId(True)

    result = sum(res['detected_character_count'].values) / sum(res['total_character'].values)

    print('Coverage of data by ctakes: ', result * 100, '%');
    # detectFromCtakesJson()
