...
import shutil
import time
from newFunctions import update_concepts_from_mlob
...
def splittingPOD():
    ...
    if not os.path.isfile(resultsDirectory + 'tododiabetes3.pckl'):
    ...
    for patient_id in list(final['patient_id'].drop_duplicates().values):
        ...
        if freetext_obs_ids:
            ...
            now, lres = update_concepts_from_mlob(todo_patient_df, now, freetext_obs_ids)
