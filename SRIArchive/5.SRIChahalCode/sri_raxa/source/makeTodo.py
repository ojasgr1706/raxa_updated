...
def fetch_concepts_from_freetext(obs_ids):
    if not obs_ids:
        ...
        cnx = mysql.connector.connect(user=userdb, password=pwddb, host=hostdb, database=databasedb)
        cursor = cnx.cursor()
        sql_query = ('SELECT value_text, concept_value from mlob where value_text is not null and value_text !="" and obs_id in (' + ','.join([str(f) for f in obs_ids]) + ')'
        ...
        if not df.empty:
            ...
            def extract_dictionary(row, res_ctakes_list_arg, key_value_dic_arg):
                ...
                if key not in list_to_avoid:
                    ...
                    if key_value is None or str(key_value) == 'nan':
