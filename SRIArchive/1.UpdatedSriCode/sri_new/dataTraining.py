...
def Num_last(x):
    if x is None or str(x) == 'nan' or isNaN(x):
        return [None, None]
    res = [None]
    try:
...
def predictionSplittedTraining():
    ...
    final = pd.read_pickle(pathToPOD + 'new_diabetes_dummy.pckl')

    # final = final[119887:129887]

    print("Diabetes Patients: ", final[final['hasDiabetesMellitus'] == 1].shape[0])
    ...
    for feature_file_name in listNum + listExtracted:
        print('Columns', feature_file_name)
    ...
    for train_index, test_index in skf.split(X_o, y_o):
        ...
        res = res.assign(predDiabetes=clf.predict(X))
        ...
        # Export Decision Tree not working due to tree.dot path error
    ...
    find_confusion_matrix(allIn)

    return allIn


def find_confusion_matrix(df):
    new_df = pd.DataFrame(df.groupby(['patient_id']).mean())
    temp_df = pd.DataFrame()
    temp_df['hasDiabetesMellitus'] = new_df['hasDiabetesMellitus']
    temp_df['predDiabetes'] = new_df['predDiabetes']
    temp_df['patient_id'] = df.groupby(['patient_id']).mean().index

    temp_df['predDiabetes'] = temp_df['predDiabetes'] > 0.5
    temp_df['predDiabetes'] = temp_df['predDiabetes'].astype(int)
