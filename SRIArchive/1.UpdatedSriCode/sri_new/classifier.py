...
class MyClf():
    ...
    def __init__(self):
        ...
        #                                   random_state=56)
        self.clf = RandomForestClassifier(n_estimators=1000, criterion='entropy', n_jobs=-1, class_weight={0: 1, 1: 30},
                                          verbose=0,
                                          random_state=56)
    ...
    def features_importance(self, X_train):
        feature_importances_df = pd.DataFrame(self.clf.feature_importances_,
                                           index=X_train.columns,
                                           columns=['importance']).sort_values('importance', ascending=False)
        return feature_importances_df

    def export_decision_tree(self, feature_list):
