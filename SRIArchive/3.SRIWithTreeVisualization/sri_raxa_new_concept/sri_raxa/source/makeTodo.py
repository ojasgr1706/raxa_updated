...
def add_concept_form_db(pathToCCIs):
    ...
    if ('patient_id' not in listPatient.columns):
        ...
        patient_list = np.unique(listPatient['patient_id'].values)
    ...
    final_patientlist = patient_list.tolist() + np.unique(res['patient_id'].values).tolist()

    todo = pd.DataFrame()
    todo['patient_id'] = final_patientlist
    todo = todo.dropna()
...
def fetch_concepts_from_freetext(obs_ids):
    if not obs_ids:
        ...
        cnx = mysql.connector.connect(user=userdb, password=pwddb, host=hostdb, database=databasedb)
        cursor = cnx.cursor()
        sql_query = ('SELECT value_text, concept_value from mlob where value_text is not null and value_text !="" and obs_id in (' + ','.join(
                [str(f) for f in obs_ids]) + ')'
                    + ' and concept_id not in (33334452, 33334004, 33334003, 170250004, 170250005) ')
